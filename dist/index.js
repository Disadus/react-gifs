...
const usePlayerState = stateOrFn => {
  return React.useReducer(reducer, stateOrFn, initializer);
};
...
const parse = (src, _ref) => {
  ...
  }).then(buffer => gifuctJs.parseGIF(buffer)).then(gif => {
    validateAndFix(gif);
    return gif;
  }).then(gif => Promise.all([gifuctJs.decompressFrames(gif, false), {
    width: gif.lsd.width,
...
const useEventCallback = callback => {
  ...
  return React.useCallback(arg => ref.current && ref.current(arg), []);
};
...
const Canvas = /*#__PURE__*/React.forwardRef(function Canvas(_ref, ref) {
  let {
    index,
    frames,
    width,
...
exports.useWorkerParser = useWorkerParser;
